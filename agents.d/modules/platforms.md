...
kind: note
status: current
summary: Per-platform delivery specs — safe zones, caption zones, duration and aspect limits. The overlay and caption jobs measure against these.
---

# Platforms — delivery specs and safe zones

This is the reference for every platform DirtBikeX publishes to: formats, durations,
safe zones, cover mechanics, and caption limits. QC (`agents.d/modules/voice-and-render-qc.md`)
hard-checks exports against this doc, and the publishing checklist
(the publishing checklist (kept by the human who publishes)) walks it per upload. Two repo rules
derive from it:
...
check 8 (the corresponding job doc) verifies against it. Platform-native text tools
...
- Since there is no cover upload, bake the "cover" into the first frames of the Remotion
  comp — this is the one platform where the cover hypothesis ships inside the
  video file.
...
3. If a duration or caption limit changed, sweep
   the publishing checklist (kept by the human who publishes) for the same numbers.
