...
<!-- Content layer only. No beats/visual_intent table exists yet (manifest.yml stays at
     ...
     **加粗 = 字幕重点** (`*` in the rendered captions) — chosen here as a first pass, same as
     ...
     `tools/group-words.mjs` did the timing (real take, not synthesised), against `script.txt`
     next to this file — see that file for the exact word-boundary input. -->

目标时长: 待定（母版时长以 `words.json` 的最后一个词 `endMs` 为准）
...
因为接下来**30多**分钟，他们的心率能一直干到**180**左右。

什么概念？

差不多就是你在爬楼机上被多位壮汉**狠狠**跟着半个小时

**What**？

那我们再来看看**世界冠军**骑的这台**本田450**。

这玩意儿真的是现代工业的**艺术品**。

就算不骑，摆那儿都**完全**可以当幅油画。

所以各位觉得我现在说服她让我在客厅墙上挂一台**可能**吗？
